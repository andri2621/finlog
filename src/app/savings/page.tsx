...
export default function SavingsPage() {
  ...
  return (
    <div className="p-4 space-y-4">
      {/* HEADER CARD: TOTAL TABUNGAN (Matches Screenshot) */}
      <div className="relative overflow-hidden p-5 rounded-3xl bg-gradient-to-br from-[#0F3A2B] via-[#0B251D] to-[#0A101D] border border-emerald-500/40 shadow-xl">
        ...
        <div className="grid grid-cols-2 gap-2 mt-4 pt-3 border-t border-emerald-500/20">
          <button
            ...
            className="py-2.5 px-3 bg-slate-900/90 hover:bg-slate-900 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-1.5 transition-colors border border-emerald-500/30 shadow-md"
          >
      ...
      <div className="space-y-3">
        ...
        {savings.length === 0 ? (
          <div className="p-8 text-center rounded-3xl bg-slate-900/60 border border-slate-800">
            ...
            <p className="text-xs text-slate-400">Belum ada tujuan menabung yang dibuat.</p>
          </div>
        ) : (
          savings.map((goal) => {
            const percent =
            ...
            return (
              <div
                key={goal.id}
                className="p-4 rounded-3xl bg-[#0F162A] border border-slate-800/90 hover:border-slate-700 shadow-md space-y-3 transition-all"
              >
                <div className="flex items-center justify-between">
                  ...
                  <button
                    ...
                    className="py-1.5 px-3 bg-emerald-500/15 hover:bg-emerald-500/25 text-emerald-400 font-semibold rounded-xl text-xs border border-emerald-500/30 transition-colors"
                  >
                    + Setor
                  </button>
                </div>
        ...
        <div className="p-4 rounded-3xl bg-[#0F162A] border border-slate-800 shadow-md space-y-2.5">
          ...
          <div className="space-y-2">
            {savingsLogs.slice(0, 5).map((log) => (
              ...
              >
                <div>
                  <p className="font-semibold text-white">{log.savingsName}</p>
                  <p className="text-[10px] text-slate-400">
                    {log.date} • {log.pocket} • Oleh {log.recordedBy}
                  </p>
                </div>
                <p className="font-bold text-emerald-400">+{formatIDR(log.amount)}</p>
              </div>
            ))}
          </div>
        </div>
