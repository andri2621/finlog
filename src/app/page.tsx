        ...
        <div className="flex items-center justify-between">
          ...
          {activeDayData ? (
            <button
              ...
              className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/20 px-2 py-0.5 rounded-md flex items-center gap-1.5 transition-colors cursor-pointer"
              title="Reset pilihan"
            >
              ...
              <Tooltip
                ...
                content={({ active, payload }) => {
                  if (active && payload && payload.length) {
                    ...
                    return (
                      <div className="bg-slate-900 text-white border border-slate-700 p-2 rounded-xl text-xs shadow-xl pointer-events-none">
              ...
              <Bar dataKey="amount" radius={[4, 4, 0, 0]} className="outline-none focus:outline-none">
                {dailyChartData.map((entry, index) => {
                  ...
                  return (
                    <Cell
                      key={`cell-${index}`}
                      fill={entry.amount > 0 ? (isSelected ? "#34D399" : "#10B981") : "#E2E8F0"}
